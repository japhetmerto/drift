<?php
	...
	$connection = new mysqli($hn, $un, $pw, $db);
 	...
 	// First 4 Games
 	$_SESSION["firstFourGames"] = "";
 	$query = "SELECT * FROM product_details, average_rating, store_game WHERE product_details.product_id = store_game.product_id AND product_details.product_id = average_rating.product_id ORDER BY average_rating.average_rating DESC LIMIT 4";
	...
	for ($j = 0; $j < $rows; ++$j) {
		...
		$_SESSION["firstFourGames"] .= "<div class='col-md-3 col-sm-6'>
	...
	// First other 4 Games
	$_SESSION["otherFourGames"] = "";
 	$query = "SELECT * FROM product_details, average_rating, store_game WHERE product_details.product_id = store_game.product_id AND product_details.product_id = average_rating.product_id ORDER BY average_rating.average_rating DESC LIMIT 4, 4";
	$result = $connection -> query($query);
	...
	for ($j = 0; $j < $rows; ++$j) {
		...
		$_SESSION["otherFourGames"] .= "<div class='col-md-3 col-sm-6'>
				<div class='product-grid3'>
					<div class='product-image3'>
						<a href='productdetails.php?product_id=" . $row["product_id"] . "&type=" . $row["type"] . "&variants=" . str_replace(" ", "%20", $row['variants']) . "'>
							<img class='pic-1' src='assets/img/content/product/$lastItem'>
