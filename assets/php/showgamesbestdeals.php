<?php
	...
	$connection = new mysqli($hn, $un, $pw, $db);
 	...
 	// First 4 Games
 	$_SESSION["firstFourGamesDeals"] = "";
 	$query = "SELECT * FROM product_details, store_game, average_rating WHERE product_details.product_id = store_game.product_id AND product_details.product_id = average_rating.product_id AND store_game.discount_offer > 0 ORDER BY store_game.discount_offer DESC LIMIT 4";
	...
	for ($j = 0; $j < $rows; ++$j) {
		...
		$_SESSION["firstFourGamesDeals"] .= "<div class='col-md-3 col-sm-6'>
	...
	// First other 4 Games
	$_SESSION["otherFourGamesDeals"] = "";
 	$query = "SELECT * FROM product_details, store_game, average_rating WHERE product_details.product_id = store_game.product_id AND product_details.product_id = average_rating.product_id AND store_game.discount_offer > 0 ORDER BY store_game.discount_offer DESC LIMIT 4, 4";
	$result = $connection -> query($query);
	...
	for ($j = 0; $j < $rows; ++$j) {
		...
		$_SESSION["otherFourGamesDeals"] .= "<div class='col-md-3 col-sm-6'>
				<div class='product-grid3'>
					<div class='product-image3'>
						<a href='productdetails.php?product_id=" . $row["product_id"] . "&type=" . $row["type"] . "&variants=" . str_replace(" ", "%20", $row['variants']) . "'>
							<img class='pic-1' src='assets/img/content/product/$lastItem'>
