<?php
	...
	$connection = new mysqli($hn, $un, $pw, $db);
 	...
 	// First 4 Console
 	$_SESSION["firstSearchConsolesAll"] = "";
 	$query = "SELECT * FROM product_details, store_console, average_rating 
WHERE product_details.product_id = store_console.product_id 
AND product_details.product_id = average_rating.product_id 
AND store_console.product_name LIKE '%{$_SESSION["searchquery"]}%' 
ORDER BY product_details.product_name LIMIT 3";
	...
	for ($j = 0; $j < $rows; ++$j) {
		...
		$_SESSION["firstSearchConsolesAll"] .= "<div class='col-md-4 col-sm-6'>
	...
	$_SESSION["otherSearchConsolesAll"] = "";
 	$query = "SELECT * FROM product_details, store_console, average_rating 
WHERE product_details.product_id = store_console.product_id 
AND product_details.product_id = average_rating.product_id 
AND store_console.product_name LIKE '%{$_SESSION["searchquery"]}%' 
ORDER BY product_details.product_name LIMIT 3, 999999999";
	...
	for ($i = 0; $i < $column; ++$i ) {
		$_SESSION["otherSearchConsolesAll"] .= "<div class='row mt-5 mb-5'>";
		...
		for ($j = 0; $j < $loop; ++$j) {
			...
			$_SESSION["otherSearchConsolesAll"] .= "<div class='col-md-4 col-sm-6'>
					<div class='product-grid3'>
						<div class='product-image3'>
							<a href='productdetails.php?product_id=" . $row["product_id"] . "&type=" . $row["type"] . "&variants=" . str_replace(" ", "%20", $row['variants']) . "'>
								<img class='pic-1' src='assets/img/content/product/$lastItem'>
		...
		$_SESSION["otherSearchConsolesAll"] .= "</div>";
	}
