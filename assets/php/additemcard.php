<?php
	...
	$connection = new mysqli($hn, $un, $pw, $db);
 	...
 	if (isset($_SESSION["username"])) {
	 	if (isset($_GET['method']) && $_GET['method'] == "addcart") {
			...
			$full_price = $row["full_price"] + ($_GET['price'] * $quantity);
			echo "full_price: $full_price <br>";

			// discount
			$discount = $row["discount"] + (($_GET['price'] - $_GET['discount_price']) * $quantity);
			...
			$product_name = $_POST['product_name'] . " - " . $_GET['variants'] . " - " . $_GET['type'];
