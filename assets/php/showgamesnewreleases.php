<?php
	...
	for ($j = 0; $j < $rows; ++$j) {
		...
		$_SESSION["firstFourGamesNew"] .= "<div class='col-md-3 col-sm-6'>
	...
	// First other 4 Games
	$_SESSION["otherFourGamesNew"] = "";
 	$query = "SELECT * FROM product_details, store_game, average_rating WHERE product_details.product_id = store_game.product_id AND product_details.product_id = average_rating.product_id ORDER BY DATE(product_details.date_release) DESC LIMIT 4, 4";
	$result = $connection -> query($query);
	...
	for ($j = 0; $j < $rows; ++$j) {
		...
		$_SESSION["otherFourGamesNew"] .= "<div class='col-md-3 col-sm-6'>
				<div class='product-grid3'>
					<div class='product-image3'>
						<a href='productdetails.php?product_id=" . $row["product_id"] . "&type=" . $row["type"] . "&variants=" . str_replace(" ", "%20", $row['variants']) . "'>
							<img class='pic-1' src='assets/img/content/product/$lastItem'>
