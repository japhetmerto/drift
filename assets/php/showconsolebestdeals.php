<?php
	...
	$connection = new mysqli($hn, $un, $pw, $db);
 	...
 	// First 4 Console
 	$_SESSION["firstFourConsoleDeals"] = "";
 	$query = "SELECT * FROM product_details, store_console, average_rating WHERE product_details.product_id = store_console.product_id AND product_details.product_id = average_rating.product_id AND store_console.discount_offer > 0 ORDER BY store_console.discount_offer DESC LIMIT 4";
	...
	for ($j = 0; $j < $rows; ++$j) {
		...
		$_SESSION["firstFourConsoleDeals"] .= "<div class='col-md-3 col-sm-6'>
	...
	// First other 4 Games
	$_SESSION["otherFourConsoleDeals"] = "";
 	$query = "SELECT * FROM product_details, store_console, average_rating WHERE product_details.product_id = store_console.product_id AND product_details.product_id = average_rating.product_id AND store_console.discount_offer > 0 ORDER BY store_console.discount_offer DESC LIMIT 4, 4";
	$result = $connection -> query($query);
	...
	for ($j = 0; $j < $rows; ++$j) {
		...
		$_SESSION["otherFourConsoleDeals"] .= "<div class='col-md-3 col-sm-6'>
				<div class='product-grid3'>
					<div class='product-image3'>
						<a href='productdetails.php?product_id=" . $row["product_id"] . "&type=" . $row["type"] . "&variants=" . str_replace(" ", "%20", $row['variants']) . "'>
							<img class='pic-1' src='assets/img/content/product/$lastItem'>
