<?php
	...
	$archivemonth = $_GET["month"];
	$archiveyear = $_GET["year"];

	$_SESSION["queryarchive"] = "SELECT * FROM articles WHERE MONTHNAME(date) = '$archivemonth' AND Year(date) = '$archiveyear' ORDER BY DATE(date) DESC";

	require_once "assets/php/shownewsmonthyear.php";
	...
	<section class="content-section" style="background: linear-gradient(to bottom, #000000 0%,#4f4c4c 100%);">
		...
		<main class="container">
		  <div class="row">
			<div class="col-md-8">
			  <!--- News -->
			  <h1 class="text-white"><b class="text-warning"><?php echo "$archivemonth $archiveyear"; ?></b></h1>
				<hr class="bg-white">
					<?php echo $_SESSION["shownewsarchive"]; ?>
